...
async function main() {
  ...
  const imgBitmap = await loadImageBitmap(
    "https://webgpufundamentals.org/webgpu/resources/images/pexels-chevanon-photography-1108099.jpg",
  );
  ...
  // We don't need to read the results back anymore
  ...
  function drawHistogram(channels, height = 100) {
    ...
    const subpart = (view, offset, length) =>
      view.subarray(offset, offset + length);
    ...
    const bindGroup = device.createBindGroup({
      ...
      entries: [
        {
          binding: 0,
          resource: { buffer: chunksBuffer, size: chunkSize * 4 * 4 },
        },
        { binding: 1, resource: uniformBuffer },
